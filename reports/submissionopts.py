...
submission_options = {
    "created": ("Date and time created", lambda sub:write_str(sub.created)),
    "track": ("Track",
    ...
    "speaker_comms": ("Communication", lambda sub:write_str(sub.communication)),
    "expenses": ("Expenses",
        lambda sub:write_option(sub.key.parent().get().expenses_options(), sub.expenses)),
    "withdrawn": ("Withdrawn", lambda sub:write_str(sub.withdrawn)),
    "grdp_agreed": ("Agreed GDPR policy", lambda sub:write_str(sub.gdpr_agreed_flag)),
    }
